...
function PIDGen()
{
	...
	if(mysql_num_rows($result) > 0)
	{
		...
		{
			$dirtyID = preg_replace('/\D/', '', $row['fldPatientID']);
			$pid = $dirtyID;
			...
			elseif(intval($arr['max']) < $pid)
			{
				$arr['max'] = $pid + 1;
			}
		}
		//if($arr['max'] < 250001) $arr['max'] = 25000;
		#2147483648
	}
	else
	{
	...
	//increase 1
	$pid = "{$arr['max']}";
	/* $sql = 'SELECT CONVERT(fldPatientID, UNSIGNED) AS fldPatientID FROM tblorderdetails ORDER BY CONVERT(fldPatientID, UNSIGNED) DESC  LIMIT 0, 1';

	if($_REQUEST['fldID'])
	{
	# Attempt to conserve keys
	$pid = strval($_REQUEST['fldID']);

	}
	else
	{
	$sql = 'SELECT MAX(fldID) AS ID FROM tblorderdetails';
...
if($_REQUEST['submit']!='')
{
	...
	if($submit != ''):
		...
		if($_REQUEST['billfacility']): $billed = 'facility';
		elseif($_REQUEST['billinsurance']): $billed = 'insurance';
		endif;
		...
		if($sql_insert):
			...
			//if($sql_insert_icd) include_once "pdf_neworder.php";//FIXME this is broken
	...
	if($id != ""):
		if($_REQUEST['neworder']==1 && $submit==''):
			...
			$sql = "select fldFirstName,fldLastName,fldMiddleName,fldSurName,fldDOB,fldGender,fldPatientSSN,fldPatientID,fldSurName,fldGender,fldFacilityName,fldLocFacName,fldStation,fldOrderingPhysicians $address FROM tblorderdetails where fldID = '$id'";
			$id = '';
		...
		$num = mysql_num_rows($result)?mysql_num_rows($result):0;
		
		if($sql_values_fetch['fldOrderType'] > 0 & $type < 1 ) $type = $sql_values_fetch['fldOrderType'];
		endif;
...
<?exit(0);
		...
		<tr>
			<td colspan="3">
				<select name="locfacility" id="locfacility" class="required" onchange="showAddress(this.value);showStations(this.value);">
					<option selected="selected" value="">Select</option>
				<?if($_SESSION['role'] === 'facilityuser' || $_SESSION['role'] === 'orderingphysician'): $sql="select fldFacility AS fldFacilityName FROM tbluserfacdetails where fldUserID = '$uid'";
	...
	if($type!=4):?>
		<tr><td colspan='8'>&nbsp;<hr/></td></tr>
		<?if(empty($sql_values_fetch['fldPatientID'])):?>
		...
		<tr>
			...
			<td colspan="2">
				...
				<select name="orderingphysicians" class="required" onChange="phyenable();showDr(this.value);">
					<option selected="selected" value="">Select</option>
				<?while($row = mysql_fetch_array($result)):?>
					<option value="<?=$row['fldRealName']?>" <?=($sql_values_fetch['fldOrderingPhysicians'] === $row['fldRealName'])?'selected="selected"':'';?>><?=strtoupper($row['fldRealName'])?></option>
				<?endwhile;?>
					<option value="new">Not In List</option>
				</select>
				<input name="phynew" id="phynew" style="display: none;"/>
			</td>
		</tr>
		<tr>
			<td><span class="lab">Phone</span></td>
	...
	if($type!=2):
		...
		<tr>
			...
			<td><input type='checkbox' name='billinsurance' <?=($sql_values_fetch['fldbilled'] === 'insurance')?'selected="selected"':''?>></td>
			<td class="lab">Bill Facility</td>
			<td><input type='checkbox' name='billfacility' <?=($sql_values_fetch['fldbilled'] === 'facility')?'selected="selected"':''?>></td>
		</tr>
		<?endif;?>
		<tr>
		...
		<tr>
			...
			<td>
				...
				<?$statesql = "select * from tblstates";
				$stateres = mysql_query($statesql);
				$states = mysql_fetch_assoc($stateres);
		...
		<tr>
			<td colspan="8">
				<div align="center">
					...
					<input name="savennew" type="submit" value="ADD & Start New <?=$typesqlarray[$type]?> Order" />
					<input name="savecnew" type="submit" value="Add & Create new order for same patient"/>
				</div>
			</td>
		</tr>
	<?endif;?>
